...
import { createTechnology as api_submissions_create_technology_createTechnology } from "~backend/submissions/create_technology";
import { list as api_submissions_list_list } from "~backend/submissions/list";
import { searchTechnologies as api_submissions_search_technologies_searchTechnologies } from "~backend/submissions/search_technologies";
...
export namespace submissions {
    ...
    export class ServiceClient {
         ...
         */
        public async createTechnology(params: RequestType<typeof api_submissions_create_technology_createTechnology>): Promise<ResponseType<typeof api_submissions_create_technology_createTechnology>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/technologies`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_submissions_create_technology_createTechnology>
        }

        /**
         * Retrieves all job submissions with optional filtering and pagination.
         */
        public async list(params: RequestType<typeof api_submissions_list_list>): Promise<ResponseType<typeof api_submissions_list_list>> {
            // Convert our params into the objects we need for the request
            const query = makeRecord<string, string | string[]>({
                page:          params.page === undefined ? undefined : String(params.page),
                perPage:       params.perPage === undefined ? undefined : String(params.perPage),
                q:             params.q,
                salaryMax:     params.salaryMax === undefined ? undefined : String(params.salaryMax),
                salaryMin:     params.salaryMin === undefined ? undefined : String(params.salaryMin),
                technologyIds: params.technologyIds?.map((v) => v),
            })

            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/submissions`, {query, method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_submissions_list_list>
        }

        /**
         * Searches for technologies by name with autocomplete functionality.
         */
        public async searchTechnologies(params: RequestType<typeof api_submissions_search_technologies_searchTechnologies>): Promise<ResponseType<typeof api_submissions_search_technologies_searchTechnologies>> {
            // Convert our params into the objects we need for the request
            const query = makeRecord<string, string | string[]>({
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/technologies/search`, {query, method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_submissions_search_technologies_searchTechnologies>
        }
    }
}
